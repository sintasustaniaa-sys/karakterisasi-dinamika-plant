•	KODINGAN KONTROL Interval Tipe-2 ANFIS
#include <Wire.h>
...
#include "esp_wpa2.h"
#include <math.h>
#include "IT2_ANFIS_params_FIKSSS.h"
...
float fireBranch(float x1,float x2,const float* c1,const float* c2,const float* s1,const float* s2,const float* p,uint8_t n){
  ...
  return num/(ws+1e-10f);
}
float infer(float x1,float x2,const float* c1,const float* c2,const float* s1u,const float* s1l,
            const float* s2u,const float* s2l,const float* p,float omin,float omax,uint8_t n){
  return omin+(0.5f*fireBranch(x1,x2,c1,c2,s1u,s2u,p,n)+0.5f*fireBranch(x1,x2,c1,c2,s1l,s2l,p,n))*(omax-omin);
}
void computeANFIS(){
  float x1,x2;
  x1=constrain(e_T,-10,10); x2=constrain(de_T,-3,3);
  float o_T=infer(x1,x2,IT2_Suhu_c1,IT2_Suhu_c2,IT2_Suhu_s1u,IT2_Suhu_s1l,IT2_Suhu_s2u,IT2_Suhu_s2l,IT2_Suhu_p,IT2_Suhu_out_min,IT2_Suhu_out_max,IT2_Suhu_N_MF);
  if(fabsf(e_T)>0.10f){ pwmK=(e_T<0)?(uint8_t)constrain(o_T,0,255):0; if(e_T<-1.0f && pwmK>0 && pwmK<60) pwmK=60; } else pwmK=0;
  x1=constrain(e_ec,-2,2); x2=constrain(de_ec,-0.5f,0.5f);
  dlyEC=(e_ec>0.05f)?(uint32_t)constrain(infer(x1,x2,IT2_EC_c1,IT2_EC_c2,IT2_EC_s1u,IT2_EC_s1l,IT2_EC_s2u,IT2_EC_s2l,IT2_EC_p,IT2_EC_out_min,IT2_EC_out_max,IT2_EC_N_MF),0,40000):0;
  x1=constrain(e_ph,-2,2); x2=constrain(de_ph,-0.5f,0.5f);
  dlyPH=(uint32_t)constrain(
    e_ph>0.05f  ? infer(x1,x2,IT2_pH_Up_c1,IT2_pH_Up_c2,IT2_pH_Up_s1u,IT2_pH_Up_s1l,IT2_pH_Up_s2u,IT2_pH_Up_s2l,IT2_pH_Up_p,IT2_pH_Up_out_min,IT2_pH_Up_out_max,IT2_pH_Up_N_MF) :
    e_ph<-0.05f ? infer(x1,x2,IT2_pH_Down_c1,IT2_pH_Down_c2,IT2_pH_Down_s1u,IT2_pH_Down_s1l,IT2_pH_Down_s2u,IT2_pH_Down_s2l,IT2_pH_Down_p,IT2_pH_Down_out_min,IT2_pH_Down_out_max,IT2_pH_Down_N_MF) : 0,
    0,22000);
  x1=constrain(e_do,-3,2); x2=constrain(de_do,-0.5f,0.5f);
pwmA=(fabsf(e_do)>0.05f)?(uint8_t)constrain(infer(x1,x2,IT2_DO_c1,IT2_DO_c2,IT2_DO_s1u,IT2_DO_s1l,IT2_DO_s2u,IT2_DO_s2l,IT2_DO_p,IT2_DO_out_min,IT2_DO_out_max,IT2_DO_N_MF),0,255):0;
}
void connectWifi(){
...
void loop(){
  ...
  if(!mq.connected()) mq.connect("TA_SINTA_IT2");
  mq.loop();
  handleActuator();
